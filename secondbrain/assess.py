...
``secondbrain assess`` indexes a project read-only and writes a before/after report a user can
...
def render_markdown(r: dict) -> str:
    ...
    out = [
        ...
        "",
        "Read-only snapshot. Re-run after changes; use `secondbrain gate` to catch drift.",
        "",
        "## Scale",
        "",
    ...
    out += ["", "*Generated by `secondbrain assess` (read-only).*", ""]
    return "\n".join(out)
